...
SEXP processx3_poll(SEXP statuses, SEXP ms) {
  ...
  pollables = (processx3_pollable_t*)
    R_alloc(num_proc * 2, sizeof(processx3_pollable_t));
  ...
  for (i = 0; i < num_proc; i++) {
    ...
    processx3_c_pollable_from_connection(&pollables[i*2], handle->pipes[1]);
    processx3_c_pollable_from_connection(&pollables[i*2+1], handle->pipes[2]);
    SET_VECTOR_ELT(result, i, allocVector(INTSXP, 2));
  }

  processx3_c_connection_poll(pollables, num_proc * 2, cms);
