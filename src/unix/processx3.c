...
static void processx3__child_init(processx3_handle_t *handle, int pipes[3][2],
				 char *command, char **args, int error_fd,
				 const char *std_out, const char *std_err,
				 processx3_options_t *options);

static SEXP processx3__make_handle(SEXP private, int cleanup);
static void processx3__handle_destroy(processx3_handle_t *handle);
...
extern processx3__child_list_t child_list_head;
extern processx3__child_list_t *child_list;
extern processx3__child_list_t child_free_list_head;
extern processx3__child_list_t *child_free_list;
...
/* We are trying to make sure that the variables in the library are
   ...
   This function is called from `R_init_processx3`. */
...
void processx3__write_int(int fd, int err) {
...
static void processx3__child_init(processx3_handle_t* handle, int pipes[3][2],
  ...
  if (fd0 == -1) { processx3__write_int(error_fd, - errno); raise(SIGKILL); }
  ...
  if (fd1 == -1) { processx3__write_int(error_fd, - errno); raise(SIGKILL); }
  ...
  if (fd2 == -1) { processx3__write_int(error_fd, - errno); raise(SIGKILL); }

  if (fd2 != 2) fd2 = dup2(fd2, 2);
  if (fd2 == -1) { processx3__write_int(error_fd, - errno); raise(SIGKILL); }

  processx3__nonblock_fcntl(fd0, 0);
  processx3__nonblock_fcntl(fd1, 0);
  processx3__nonblock_fcntl(fd2, 0);
  ...
  execvp(command, args);
  processx3__write_int(error_fd, - errno);
  raise(SIGKILL);
}
...
void processx3__finalizer(SEXP status) {
  ...
  if (handle->cleanup) {
    ...
    if (wp == pid) processx3__collect_exit_status(status, wstat);
	...
	wp = waitpid(pid, &wstat, 0);
      ...
      processx3__collect_exit_status(status, wstat);
    }
  }
...
void processx3__make_socketpair(int pipe[2]) {
   ...
   */
  if (errno != EINVAL) {
    error("processx3 socketpair: %s", strerror(errno)); /* LCOV_EXCL_LINE */
  }
...
skip:
  ...
  if (socketpair(AF_UNIX, SOCK_STREAM, 0, pipe)) {
    error("processx3 socketpair: %s", strerror(errno));
  }

  processx3__cloexec_fcntl(pipe[0], 1);
  processx3__cloexec_fcntl(pipe[1], 1);
}
...
SEXP processx3_exec(SEXP command, SEXP args, SEXP std_out, SEXP std_err,
  ...
  char *ccommand = processx3__tmp_string(command, 0);
  char **cargs = processx3__tmp_character(args);
  ...
  processx3_handle_t *handle = NULL;
  SEXP result;

  if (pipe(signal_pipe)) { goto cleanup; }
  processx3__cloexec_fcntl(signal_pipe[0], 1);
  processx3__cloexec_fcntl(signal_pipe[1], 1);

  processx3__setup_sigchld();
  ...
  if (cstdout && !strcmp(cstdout, "|")) processx3__make_socketpair(pipes[1]);
  if (cstderr && !strcmp(cstderr, "|")) processx3__make_socketpair(pipes[2]);
  ...
  if (pid == 0) {
    /* LCOV_EXCL_START */
    processx3__child_init(handle, pipes, ccommand, cargs, signal_pipe[1],
			 cstdout, cstderr, &options);
    goto cleanup;
    /* LCOV_EXCL_STOP */
  }

  /* We need to know the processx3 children */
  if (processx3__child_add(pid, result)) {
    err = -errno;
    if (signal_pipe[0] >= 0) close(signal_pipe[0]);
    if (signal_pipe[1] >= 0) close(signal_pipe[1]);
    processx3__unblock_sigchld();
  ...
  if (pipes[1][0] >= 0) {
    ...
    processx3__nonblock_fcntl(handle->fd1, 1);
  }
  if (pipes[2][0] >= 0) {
    handle->fd2 = pipes[2][0];
    processx3__nonblock_fcntl(handle->fd2, 1);
  }
  ...
  processx3__create_connections(handle, private, cencoding);
...
SEXP processx3_wait(SEXP status, SEXP timeout) {
  ...
  if (!handle) {
    processx3__unblock_sigchld();
    error("Internal processx3 error, handle already removed");
  }
  ...
  if (pipe(handle->waitpipe)) {
    processx3__unblock_sigchld();
    error("processx3 error: %s", strerror(errno));
  }
  processx3__nonblock_fcntl(handle->waitpipe[0], 1);
  processx3__nonblock_fcntl(handle->waitpipe[1], 1);
  ...
  while (ctimeout < 0 || timeleft > PROCESSX3_INTERRUPT_INTERVAL) {
    do {
      ret = poll(&fd, 1, PROCESSX3_INTERRUPT_INTERVAL);
    ...
    if (ctimeout >= 0) timeleft -= PROCESSX3_INTERRUPT_INTERVAL;
  }
  ...
  if (ret == -1) {
    error("processx3 wait with timeout error: %s", strerror(errno));
  }
...
/* This is similar to `processx3_wait`, but a bit simpler, because we
...
SEXP processx3_is_alive(SEXP status) {
  ...
  if (wp == -1) {
    processx3__unblock_sigchld();
    error("processx3_is_alive: %s", strerror(errno));
  }
...
/* This is essentially the same as `processx3_is_alive`, but we return an
...
SEXP processx3_get_exit_status(SEXP status) {
  ...
  if (wp == -1) {
    processx3__unblock_sigchld();
    error("processx3_get_exit_status: %s", strerror(errno));
  }
...
/* See `processx3_wait` above for the description of async processes and
 * possible race conditions.
 *
 * This is mostly along the lines of `processx3_is_alive`. After we
 * successfully sent the signal, we try a `waitpid` just in case the
 * processx3 has aborted on it. This is a harmless race condition, because
...
SEXP processx3_signal(SEXP status, SEXP signal) {
  ...
  if (ret == 0) {
    ...
    processx3__unblock_sigchld();
    error("processx3_signal: %s", strerror(errno));
    return R_NilValue;
  }
...
/* This is a special case of `processx3_signal`, and we implement it almost
...
SEXP processx3_kill(SEXP status, SEXP grace) {
  ...
  if (wp == -1) {
    processx3__unblock_sigchld();
    error("processx3_kill: %s", strerror(errno));
  }
