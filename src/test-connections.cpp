...
context("Basics") {
  ...
  test_that("can create a connection from os handle") {
    processx3_file_handle_t handle = open_file("fixtures/simple.txt");
    processx3_connection_t *ccon =
      processx3_c_connection_create(handle, PROCESSX3_FILE_TYPE_ASYNCFILE, "UTF-8", 0);
    expect_true(ccon != 0);
...
context("Reading characters") {
  ...
  test_that("can read characters and set EOF") {
    ...
    expect_false(processx3_c_connection_is_eof(ccon));

    processx3_pollable_t pollable;
    processx3_c_pollable_from_connection(&pollable, ccon);

    char buffer[10];
    processx3_c_connection_poll(&pollable, 1, -1);
    size_t ret = processx3_c_connection_read_chars(ccon, buffer, 10);
    expect_true(ret == 10);
    expect_true(! strncmp(buffer, "simple tex", 10));

    expect_false(processx3_c_connection_is_eof(ccon));

    ret = processx3_c_connection_read_chars(ccon, buffer, 10);
    ...
    expect_true(processx3_c_connection_is_eof(ccon));
  ...
  test_that("EOF edge case") {
    ...
    char buffer[18];
    size_t ret = processx3_c_connection_read_chars(ccon, buffer, 18);
    expect_true(ret >= 17);
    expect_true(ret <= 18);
    ...
    ret = processx3_c_connection_read_chars(ccon, buffer, 17);
    expect_true(ret == 0);
    expect_true(processx3_c_connection_is_eof(ccon));
  ...
  test_that("A larger file that needs buffering") {
    char *filename;
    processx3_file_handle_t handle = open_temp_file(&filename, 100000, 0);
    processx3_connection_t *ccon =
      processx3_c_connection_create(handle, PROCESSX3_FILE_TYPE_ASYNCFILE, "UTF-8", 0);
    ...
    char buffer[1024];
    while (! processx3_c_connection_is_eof(ccon)) {
      size_t ret = processx3_c_connection_read_chars(ccon, buffer, 1024);
      if (ret == 0) expect_true(processx3_c_connection_is_eof(ccon));
    }
  ...
  test_that("Reading UTF-8 file") {
    ...
    char buffer[4];
    ssize_t ret = processx3_c_connection_read_chars(ccon, buffer, 4);
    expect_true(ret == 2);
    expect_true(buffer[0] == '\xc2');
    expect_true(buffer[1] == '\xa0');

    ret = processx3_c_connection_read_chars(ccon, buffer, 4);
    expect_true(ret == 3);
  ...
  test_that("Conversion to UTF-8") {
    ...
    processx3_file_handle_t handle = open_temp_file(&filename, 1, latin1);

    processx3_connection_t *ccon =
      processx3_c_connection_create(handle, PROCESSX3_FILE_TYPE_ASYNCFILE, "latin1", 0);

    expect_false(processx3_c_connection_is_eof(ccon));

    char buffer[10];
    ssize_t ret = processx3_c_connection_read_chars(ccon, buffer, 10);
    expect_true(ret == 6);
    buffer[6] = '\0';
...
context("Reading lines") {
  ...
  test_that("Reading a line") {
    char *filename;
    processx3_file_handle_t handle = open_temp_file(&filename, 50, "hello\n");
    processx3_connection_t *ccon =
      processx3_c_connection_create(handle, PROCESSX3_FILE_TYPE_ASYNCFILE, "UTF-8", 0);

    char *linep = 0;
    size_t linecapp = 0;
    ssize_t read = processx3_c_connection_read_line(ccon, &linep, &linecapp);
    expect_true(read == 5);
  ...
  test_that("Reading the last incomplete line") {
    char *filename;
    processx3_file_handle_t handle = open_temp_file(&filename, 1, "hello\nhello\nagain");
    processx3_connection_t *ccon =
      processx3_c_connection_create(handle, PROCESSX3_FILE_TYPE_ASYNCFILE, "UTF-8", 0);
    ...
    for (int i = 0; i < 2; i++) {
      read = processx3_c_connection_read_line(ccon, &linep, &linecapp);
    ...
    read = processx3_c_connection_read_chars(ccon, linep, 4);
    expect_true(read == 0);
    expect_true(processx3_c_connection_is_eof(ccon));
