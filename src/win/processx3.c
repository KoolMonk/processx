...
static HANDLE processx3__global_job_handle = NULL;

static void processx3__init_global_job_handle(void) {
  /* Create a job object and set it up to kill all contained processes when
   * it's closed. Since this handle is made non-inheritable and we're not
   * giving it to anyone, we're the only process holding a reference to it.
   * That means that if this process exits it is closed and all the
   * processes it contains are killed. All processes created with processx3
   ...
   * explicitly breaking away from job control (which processx3_exec
   * doesn't do, either). */
  ...
  processx3__global_job_handle = CreateJobObjectW(&attr, NULL);
  if (processx3__global_job_handle == NULL) {
    PROCESSX3_ERROR("Creating global job object", GetLastError());
  }

  if (!SetInformationJobObject(processx3__global_job_handle,
                               JobObjectExtendedLimitInformation,
                               &info,
                               sizeof info)) {
    PROCESSX3_ERROR("Setting up global job object", GetLastError());
  }
}
...
SEXP processx3__killem_all() {
  if (processx3__global_job_handle) {
    TerminateJobObject(processx3__global_job_handle, 1);
    CloseHandle(processx3__global_job_handle);
    processx3__global_job_handle = NULL;
  }
  return R_NilValue;
}
...
int processx3__utf8_to_utf16_alloc(const char* s, WCHAR** ws_ptr) {
  ...
  if (r != ws_len) {
    error("processx3 error interpreting UTF8 command or arguments");
  }
...
WCHAR* processx3__quote_cmd_arg(const WCHAR *source, WCHAR *target) {
...
static int processx3__make_program_args(SEXP args, int verbatim_arguments,
				       WCHAR **dst_ptr) {
  const char* arg;
  WCHAR* dst = NULL;
  ...
  for (i = 0; i < arg_count; i++) {
    ...
    if (verbatim_arguments) {
      ...
      pos = processx3__quote_cmd_arg(temp_buffer, pos);
    }
...
static WCHAR* processx3__search_path_join_test(const WCHAR* dir,
					      size_t dir_len,
					      const WCHAR* name,
					      size_t name_len,
					      const WCHAR* ext,
 ...
 */
static WCHAR* processx3__path_search_walk_ext(const WCHAR *dir,
					     size_t dir_len,
					     const WCHAR *name,
					     size_t name_len,
					     WCHAR *cwd,
  ...
  if (name_has_ext) {
    result = processx3__search_path_join_test(dir, dir_len,
					     name, name_len,
					     L"", 0,
					     cwd, cwd_len);
    if (result != NULL) {
 ...
 *
 */
static WCHAR* processx3__search_path(const WCHAR *file,
				    WCHAR *cwd,
				    const WCHAR *path) {
  int file_has_dir;
  WCHAR* result = NULL;
  ...
  if (file_has_dir) {
    ...
    result = processx3__path_search_walk_ext(
    ...
    result = processx3__path_search_walk_ext(L"", 0,
					    file, file_len,
					    cwd, cwd_len,
					    name_has_ext);
    ...
    while (result == NULL) {
      ...
      result = processx3__path_search_walk_ext(dir_path, dir_len,
					      file, file_len,
					      cwd, cwd_len,
					      name_has_ext);
    }
...
void processx3__error(const char *message, DWORD errorcode,
  ...
  error("processx3 error, %s: #%d %s at '%s:%d'", message,
	(int) errorcode, msg, file, line);
}

void processx3__collect_exit_status(SEXP status, DWORD exitcode);

DWORD processx3__terminate(processx3_handle_t *handle, SEXP status) {
  DWORD err;

  err = TerminateProcess(handle->hProcess, 2);
  if (err) processx3__collect_exit_status(status, 2);
...
void processx3__finalizer(SEXP status) {
  processx3_handle_t *handle = (processx3_handle_t*) R_ExternalPtrAddr(status);
  SEXP private;
  ...
  if (handle->cleanup && !handle->collected) {
    ...
    processx3__terminate(handle, status);
  }
  ...
  processx3__handle_destroy(handle);
}

SEXP processx3__make_handle(SEXP private, int cleanup) {
  processx3_handle_t * handle;
  SEXP result;

  handle = (processx3_handle_t*) malloc(sizeof(processx3_handle_t));
  if (!handle) { error("Out of memory"); }
  memset(handle, 0, sizeof(processx3_handle_t));

  result = PROTECT(R_MakeExternalPtr(handle, private, R_NilValue));
  R_RegisterCFinalizerEx(result, processx3__finalizer, 1);
...
SEXP processx3_exec(SEXP command, SEXP args, SEXP std_out, SEXP std_err,
  ...
  err = processx3__utf8_to_utf16_alloc(CHAR(STRING_ELT(command, 0)),
				      &application);
  if (err) { PROCESSX3_ERROR("utf8 -> utf16 conversion", err); }

  err = processx3__make_program_args(
      args,
      options.windows_verbatim_args,
      &arguments);
  if (err) { PROCESSX3_ERROR("making program args", err); }
  ...
  /* Inherit cwd */
  {
    ...
    if (!cwd_len) {
      PROCESSX3_ERROR("get current directory length", GetLastError());
    }
    ...
    if (r == 0 || r >= cwd_len) {
      PROCESSX3_ERROR("get current directory", GetLastError());
    }
  }
  ...
  /* Get PATH environment variable */
  {
    ...
    if (!path_len) {
      PROCESSX3_ERROR("get env var length", GetLastError());
    }
    ...
    if (r == 0 || r >= path_len) {
      PROCESSX3_ERROR("get env var", GetLastError());
    }
  }

  result = PROTECT(processx3__make_handle(private, ccleanup));
  handle = R_ExternalPtrAddr(result);

  err = processx3__stdio_create(handle, cstd_out, cstd_err,
			       &handle->child_stdio_buffer, private,
			       cencoding);
  if (err) { PROCESSX3_ERROR("setup stdio", err); }

  application_path = processx3__search_path(application, cwd, path);
  if (!application_path) {
    R_ClearExternalPtr(result);
    processx3__stdio_destroy(handle->child_stdio_buffer);
    free(handle);
  ...
  startup.cbReserved2 = processx3__stdio_size(handle->child_stdio_buffer);
  startup.lpReserved2 = (BYTE*) handle->child_stdio_buffer;

  startup.hStdInput = processx3__stdio_handle(handle->child_stdio_buffer, 0);
  startup.hStdOutput = processx3__stdio_handle(handle->child_stdio_buffer, 1);
  startup.hStdError = processx3__stdio_handle(handle->child_stdio_buffer, 2);
  ...
  if (!ccleanup) {
     ...
     * means that processx3 might not let you create a fully deamonized
     * process when run under job control. However the type of job control
     * that processx3 itself creates doesn't trickle down to subprocesses
  ...
  if (!err) { PROCESSX3_ERROR("create process", GetLastError()); }
  ...
  if (!ccleanup) {
    if (! processx3__global_job_handle) processx3__init_global_job_handle();

    if (!AssignProcessToJobObject(processx3__global_job_handle, info.hProcess)) {
       ...
       * there would be no way for R/processx3 applications run under job
       * control to spawn processes at all.
       */
      DWORD err = GetLastError();
      if (err != ERROR_ACCESS_DENIED) {
	PROCESSX3_ERROR("Assign to job object", err);
      }
    }
  }

  dwerr = ResumeThread(info.hThread);
  if (dwerr == (DWORD) -1) PROCESSX3_ERROR("resume thread", GetLastError());
...
void processx3__collect_exit_status(SEXP status, DWORD exitcode) {
  processx3_handle_t *handle = R_ExternalPtrAddr(status);
...
SEXP processx3_wait(SEXP status, SEXP timeout) {
  ...
  err2 = WAIT_TIMEOUT;
  while (ctimeout < 0 || timeleft > PROCESSX3_INTERRUPT_INTERVAL) {
    err2 = WaitForSingleObject(handle->hProcess, PROCESSX3_INTERRUPT_INTERVAL);
    if (err2 != WAIT_TIMEOUT) break;
    R_CheckUserInterrupt();
    timeleft -= PROCESSX3_INTERRUPT_INTERVAL;
  }
  ...
  if (err2 == WAIT_FAILED) {
    PROCESSX3_ERROR("wait on process", GetLastError());
  ...
  if (!err) { PROCESSX3_ERROR("get exit code after wait", GetLastError()); }

  processx3__collect_exit_status(status, exitcode);
...
SEXP processx3_is_alive(SEXP status) {
  ...
  err = GetExitCodeProcess(handle->hProcess, &exitcode);
  if (!err) {
    PROCESSX3_ERROR("get exit code to check if alive", GetLastError());
  }
...
SEXP processx3_get_exit_status(SEXP status) {
  ...
  /* Otherwise try to get exit code */
  err = GetExitCodeProcess(handle->hProcess, &exitcode);
  if (!err) {PROCESSX3_ERROR("get exit status", GetLastError()); }
...
SEXP processx3_signal(SEXP status, SEXP signal) {
  ...
  case 2: {  /* SIGINT */
    ...
    err = GetExitCodeProcess(handle->hProcess, &exitcode);
    if (!err) {
      PROCESSX3_ERROR("get exit code after signal", GetLastError());
    }
    ...
    if (exitcode == STILL_ACTIVE) {
	 ...
	 work around pid reuse. */
      /* processx3__cleanup_child_tree(handle->dwProcessId); */
      err = processx3__terminate(handle, status);
  ...
  case 0: {
    ...
    err = GetExitCodeProcess(handle->hProcess, &exitcode);
    if (!err) {
      PROCESSX3_ERROR("get exit code for signal 0", GetLastError());
    }
...
SEXP processx3_kill(SEXP status, SEXP grace) {
  return processx3_signal(status, ScalarInteger(9));
}

SEXP processx3_get_pid(SEXP status) {
  processx3_handle_t *handle = R_ExternalPtrAddr(status);

  if (!handle) { error("Internal processx3 error, handle already removed"); }
...
SEXP processx3__process_exists(SEXP pid) {
  ...
  if (proc == NULL) {
    ...
    PROCESSX3_ERROR("open process to check if it exists", err);
    return R_NilValue;
  } else {
