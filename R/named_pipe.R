...
is_pipe_open.windows_named_pipe <- function(pipe) {
  .Call(c_processx3_is_named_pipe_open, pipe$handle)
}
...
create_named_pipe <- function(name) {
  if (is_windows()) {
    structure(
      list(
        handle = .Call(c_processx3_create_named_pipe, name, "")
      ),
...
close_named_pipe.windows_named_pipe <- function(pipe) {
  .Call(c_processx3_close_named_pipe, pipe$handle)
}
...
write_lines_named_pipe.windows_named_pipe <- function(pipe, text) {
  ...
  .Call(c_processx3_write_named_pipe, pipe$handle, text)
}
