...
reg.finalizer(supervisor_info, function(s) {
  ...
  # `processx3:::supervisor_info` has been created and the name
...
supervisor_start <- function() {
  ...
  if (!ready)
    stop("processx3 supervisor was not ready after 5 seconds.")
...
supervisor_path <- function() {
  ...
  system.file(subdir, supervisor_name, package = "processx3", mustWork = TRUE)
}
